...
function renderScores(data) {
  el('overallScore').innerText = data.overall_score ?? data.overallScore ?? '--';
  const breakdown = el('breakdown');
  breakdown.innerHTML = '';
  // display the four main quality metrics with meters (0–10 scale)
  const barMetrics = [
  ...
  for (const [label, val] of barMetrics) {
  ...
  // keep complexity metrics as plain text below
  const complexity = [
    ['Time Complexity', data.time_complexity],
    ['Space Complexity', data.space_complexity]
  ];
  for (const [label, val] of complexity) {
...
function renderIssues(issues) {
  ...
  container.innerHTML = '';
  if (!issues) { container.innerText = 'No issues found.'; return; }
  for (const sev of ['critical','high','medium','low']) {
    ...
    for (const it of arr) {
      ...
      if (typeof it === 'string') li.innerText = it;
      else li.innerText = `${it.line ? 'Line ' + it.line + ': ' : ''}${it.message || JSON.stringify(it)}`;
      ul.appendChild(li);
    }
...
function setImprovement(percent) {
  ...
  text.innerText = p >= 0 ? `Code Quality Improved by ${p}%` : `Change: ${p}%`;
}
...
document.addEventListener('DOMContentLoaded', () => {
  el('reviewBtn').addEventListener('click', async () => {
    ...
    try {
      ...
      const normalized = {
        ...
        issues: data.issues,
        improvement_suggestion: data.improvement_suggestion ?? data.improvementSuggestion,
        rewritten_code: data.rewritten_code ?? data.rewrittenCode
      };
      // show stub banner if used
      const banner = el('stubBanner');
      if (normalized.used_stub) {
        banner.classList.remove('hidden');
        banner.innerText = 'Warning: Response is a local stub. ' + (normalized._debug_error ? ('Debug: ' + normalized._debug_error) : '');
      } else {
      ...
      // show rewritten code with preserved line breaks and highlighting
      const raw = normalized.rewritten_code || '';
  ...
  el('rewriteBtn').addEventListener('click', async () => {
    const code = el('codeInput').value;
    const language = el('language').value;
    // Try to get original overall score from UI
    const origScore = Number(el('overallScore').innerText) || 0;
    try {
      const data = await postJSON('/rewrite', { code, language, original_score: origScore });
      const normalized = {
        overall_score: data.overall_score ?? data.overallScore,
        used_stub: data.used_stub ?? data.usedStub ?? false,
        _debug_error: data._debug_error ?? data._debugError ?? null,
        ...
        improvement_suggestion: data.improvement_suggestion ?? data.improvementSuggestion,
        rewritten_code: data.rewritten_code ?? data.rewrittenCode,
        improvement_percentage: data.improvement_percentage ?? data.improvementPercentage
      };
      const banner = el('stubBanner');
      if (normalized.used_stub) {
        banner.classList.remove('hidden');
      ...
      codeEl.className = `hljs language-${language}`;
      hljs.highlightElement(codeEl);
      setImprovement(normalized.improvement_percentage || 0);
    } catch (e) {
